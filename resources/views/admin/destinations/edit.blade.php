...
<div class="space-y-8 animate-fade-in pb-12">
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 border-b border-gray-100 pb-8">
        <div>
            <div class="flex items-center gap-2 text-sm text-gray-500 mb-2">
                ...
                <i class="fas fa-chevron-right text-[10px]"></i>
                <span class="text-gray-900 font-medium">Edit Destination</span>
            </div>
            <h1 class="text-3xl font-black text-gray-900 tracking-tight font-poppins">Edit Destination</h1>
            <p class="text-gray-500 mt-1">Refining the details of <span class="text-forest font-bold">{{ $destination->name }}</span></p>
        </div>
    ...
    <form id="destinationForm" action="{{ route('admin.destinations.update', $destination) }}" method="POST" enctype="multipart/form-data" class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        @csrf
        @method('PUT')
                ...
                <div class="space-y-6">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="space-y-2">
                            ...
                            <input type="text" name="name" required value="{{ old('name', $destination->name) }}" placeholder="e.g., Spiti Valley"
                        ...
                        <div class="space-y-2">
                            ...
                            <input type="text" name="location" required value="{{ old('location', $destination->location) }}" placeholder="e.g., Himachal Pradesh, India"
                                   class="w-full px-5 py-3.5 bg-gray-50 border border-transparent rounded-2xl focus:bg-white focus:border-gold focus:ring-4 focus:ring-gold/10 transition-all duration-300 font-medium">
                        </div>
                    </div>
                    ...
                    <div class="space-y-2">
                        ...
                        <textarea name="description" required rows="5" placeholder="Captivate travelers with a beautiful description..."
                                  class="w-full px-5 py-3.5 bg-gray-50 border border-transparent rounded-2xl focus:bg-white focus:border-gold focus:ring-4 focus:ring-gold/10 transition-all duration-300 font-medium resize-none">{{ old('description', $destination->description) }}</textarea>
                    </div>
                        ...
                        <div class="space-y-2">
                            ...
                            <select name="category" class="w-full px-5 py-3.5 bg-gray-50 border border-transparent rounded-2xl focus:bg-white focus:border-gold focus:ring-4 focus:ring-gold/10 transition-all duration-300 font-medium appearance-none cursor-pointer">
                                ...
                                @foreach($categories as $cat)
                                    ...
                                    <option value="{{ $cat }}" {{ old('category', $destination->category) == $cat ? 'selected' : '' }}>{{ ucfirst($cat) }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="space-y-2">
                            <label class="text-xs font-black text-gray-500 uppercase tracking-widest ml-1">Best Time to Visit</label>
                            <input type="text" name="best_time" value="{{ old('best_time', $destination->best_time) }}" placeholder="e.g., April to October"
                                   class="w-full px-5 py-3.5 bg-gray-50 border border-transparent rounded-2xl focus:bg-white focus:border-gold focus:ring-4 focus:ring-gold/10 transition-all duration-300 font-medium">
                        </div>
                    </div>
                    ...
                    <div class="space-y-2">
                        ...
                        <input type="text" name="tags" value="{{ old('tags', is_array($destination->tags) ? implode(', ', $destination->tags) : '') }}" placeholder="mountains, adventure, cold desert"
                               class="w-full px-5 py-3.5 bg-gray-50 border border-transparent rounded-2xl focus:bg-white focus:border-gold focus:ring-4 focus:ring-gold/10 transition-all duration-300 font-medium">
            ...
            <div class="bg-white rounded-3xl border border-gray-100 shadow-sm p-8">
                ...
                <div class="space-y-6">
                    <div class="space-y-2">
                        ...
                        <input type="text" name="meta_title" value="{{ old('meta_title', $destination->meta_title) }}" placeholder="Concise title for search results"
                               class="w-full px-5 py-3.5 bg-gray-50 border border-transparent rounded-2xl focus:bg-white focus:border-gold focus:ring-4 focus:ring-gold/10 transition-all duration-300 font-medium">
                    </div>
                    ...
                    <div class="space-y-2">
                        ...
                        <textarea name="meta_description" rows="3" placeholder="Engaging summary for search result snippets..."
                                  class="w-full px-5 py-3.5 bg-gray-50 border border-transparent rounded-2xl focus:bg-white focus:border-gold focus:ring-4 focus:ring-gold/10 transition-all duration-300 font-medium resize-none text-sm">{{ old('meta_description', $destination->meta_description) }}</textarea>
                    </div>

                    <div class="space-y-2">
                        <label class="text-xs font-black text-gray-500 uppercase tracking-widest ml-1">SEO Focus Keywords</label>
                        <input type="text" name="seo_keywords" value="{{ old('seo_keywords', $destination->seo_keywords) }}" placeholder="e.g., travel to spiti, best places in himachal"
                               class="w-full px-5 py-3.5 bg-gray-50 border border-transparent rounded-2xl focus:bg-white focus:border-gold focus:ring-4 focus:ring-gold/10 transition-all duration-300 font-medium">
                    </div>
                </div>
            </div>
                ...
                <div class="relative group">
                    <div class="file-upload-area relative aspect-video rounded-2xl border-2 border-dashed border-gray-100 bg-gray-50 flex flex-col items-center justify-center p-6 text-center cursor-pointer hover:border-gold hover:bg-gold/5 transition-all duration-500 overflow-hidden" 
                        ...
                        @if($destination->image)
                            <div id="currentImageDisplay" class="absolute inset-0 z-0">
                                <img src="{{ asset('storage/' . $destination->image) }}" alt="Current" class="w-full h-full object-cover">
                                <div class="absolute inset-0 bg-black/20 group-hover:bg-black/40 transition-all"></div>
                            </div>
                        @endif

                        <div id="uploadPlaceholder" class="relative z-10 transition-all duration-500 {{ $destination->image ? 'group-hover:opacity-100 opacity-0 scale-90' : '' }}">
                            ...
                            <p class="text-xs font-black text-gray-900 uppercase tracking-widest mb-1">{{ $destination->image ? 'Replace Photo' : 'Click to Upload' }}</p>
                            <p class="text-[10px] text-gray-400 font-medium">PNG, JPG up to 5MB</p>
                        </div>

                        <div id="imagePreview" class="absolute inset-0 hidden border-4 border-white shadow-inner z-20 bg-white">
                    ...
                    <button type="button" onclick="clearImage()" id="removeImageBtn" class="absolute -top-3 -right-3 w-8 h-8 bg-red-500 text-white rounded-full shadow-lg items-center justify-center hidden hover:scale-110 transition-all z-30">
                ...
                <div class="space-y-4">
                    <div class="p-4 rounded-2xl bg-gray-50 border border-gray-100 flex items-start gap-4 cursor-pointer hover:border-gold transition-all" onclick="document.getElementById('statusCheckbox').click()">
                        <div class="relative flex items-center">
                            <input type="checkbox" name="status" id="statusCheckbox" value="1" {{ old('status', $destination->status) ? 'checked' : '' }} class="w-5 h-5 rounded-md border-2 border-gray-200 text-forest focus:ring-forest transition-all">
                        </div>
                        <div class="flex-1">
                    ...
                    <div class="px-2 pt-2">
                        <div class="flex items-center justify-between text-[10px] font-black uppercase tracking-widest text-gray-400">
                            <span>Created</span>
                            <span>{{ $destination->created_at->format('M d, Y') }}</span>
                        </div>
                        <div class="flex items-center justify-between text-[10px] font-black uppercase tracking-widest text-gray-400 mt-1">
                            <span>Last Updated</span>
                            <span>{{ $destination->updated_at->diffForHumans() }}</span>
                        </div>
                    </div>
                </div>
            </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        imageInput.addEventListener('change', function(e) {
            ...
            if (file && file.type.startsWith('image/')) {
                ...
                reader.onload = function(e) {
                    ...
                    placeholder.classList.add('opacity-100', 'scale-100');
                    if(currentDisplay) currentDisplay.classList.add('opacity-0');
        ...
        window.clearImage = function() {
            ...
            if(currentDisplay) currentDisplay.classList.remove('opacity-0');
            placeholder.classList.remove('opacity-100', 'scale-100');
