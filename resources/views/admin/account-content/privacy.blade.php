...
<div class="space-y-6">
    ...
    <div class="flex justify-between items-center">
        <div>
            <h2 class="text-2xl font-bold text-[#17320b]">Manage Privacy Policy</h2>
            <p class="text-gray-600">Update the Privacy Policy content for your users</p>
        </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 space-y-6">
            <div class="card">
                <h3 class="text-lg font-bold text-[#17320b] mb-4">Edit Privacy Policy Content</h3>
                <form action="{{ route('admin.account-content.privacy.update') }}" method="POST">
                    @csrf
                    <div class="space-y-4">
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Page Title *</label>
                            <input type="text" name="title" value="{{ $privacy->title }}" required class="form-input">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Content (Markdown/HTML supported) *</label>
                            <textarea name="content" rows="15" required class="w-full form-input" style="font-family: monospace;">{{ $privacy->content }}</textarea>
                        </div>
                        ...
                        <div class="pt-4 border-t border-gray-100">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input type="text" name="meta_title" value="{{ $privacy->data['meta_title'] ?? '' }}" class="form-input" placeholder="Privacy Policy | ZUBEEE">
                                </div>
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 mb-1">Meta Description</label>
                                    <textarea name="meta_description" rows="2" class="form-input">{{ $privacy->data['meta_description'] ?? '' }}</textarea>
                                </div>
                            </div>
                        </div>
        ...
        <div class="space-y-6">
            <div class="card bg-[#17320b] text-white" style="background-color: #17320b !important;">
                <h3 class="text-lg font-bold text-[#a8894d] mb-3">Privacy Guide</h3>
                <div class="text-sm space-y-3 opacity-90 text-white">
                    <p>This content is displayed to users when they click the "Privacy Policy" link during Login or Signup.</p>
                    <ul class="list-disc pl-5 space-y-2">
                        <li>Explain what data you collect.</li>
<li>State how you use the data.</li>
                        <li>Detail third-party sharing.</li>
                    </ul>
                </div>
                <div class="mt-6 pt-4 border-t border-white/10">
                    <p class="text-xs text-gray-300">Last updated: {{ $privacy->updated_at->format('M d, Y H:i') }}</p>
                </div>
            </div>

            <div class="card">
                <h3 class="text-lg font-bold text-[#17320b] mb-3">Current Link</h3>
                <p class="text-sm text-gray-600 mb-4">Link to this page using:</p>
                <div class="bg-gray-50 p-3 rounded border border-gray-200 flex items-center justify-between">
                    <code class="text-xs text-secondary">/info/privacy</code>
                    <button class="text-gray-400 hover:text-secondary" onclick="navigator.clipboard.writeText('/info/privacy')">
