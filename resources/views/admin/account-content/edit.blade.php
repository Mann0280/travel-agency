    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
            <div class="flex items-center gap-3 mb-3">
                ...
                <h1 class="text-3xl font-black text-forest font-poppins uppercase tracking-tight">Refine Information</h1>
            </div>
            <p class="text-slate text-sm font-medium ml-4 italic">{{ $accountContent->title }}</p>
        </div>
...
<div class="max-w-4xl mx-auto">
    <form action="{{ route('admin.account-content.update', $accountContent) }}" method="POST" class="space-y-8">
        @csrf
        @method('PUT')
        ...
        <div class="bg-white rounded-2xl border border-gray-100 p-8 shadow-lg">
            <div class="flex items-center gap-3 mb-8 border-b border-gray-100 pb-4">
                ...
                <h3 class="text-xl font-black text-forest font-poppins uppercase tracking-tight">Narrative Refinement</h3>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div>
                    ...
                    <select name="type" id="type" 
                            ...
                            required>
                        <option value="faq" {{ $accountContent->type == 'faq' ? 'selected' : '' }}>FAQ (Frequently Asked)</option>
                        <option value="about" {{ $accountContent->type == 'about' ? 'selected' : '' }}>About Us Section</option>
                        <option value="policy" {{ $accountContent->type == 'policy' ? 'selected' : '' }}>Policy / Terms</option>
                    </select>
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="title" id="title" 
                           ...
                           value="{{ $accountContent->title }}" required>
                </div>
            </div>
            
            <div class="mt-8">
                <label for="content" class="block text-xs font-black text-forest uppercase tracking-widest mb-2 font-poppins">Refined Manuscript (Content)</label>
                <textarea name="content" id="content" rows="8" 
                          ...
                          required>{{ $accountContent->content }}</textarea>
            </div>
        </div>
        ...
        <div class="bg-white rounded-2xl border border-gray-100 p-8 shadow-lg">
            <div class="flex items-center gap-3 mb-8 border-b border-gray-100 pb-4">
                ...
                <h3 class="text-xl font-black text-forest font-poppins uppercase tracking-tight">Structural Synchronization</h3>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div>
                    ...
                    <input type="number" name="sort_order" id="sort_order" 
                           class="w-full px-5 py-3.5 bg-gray-50 border border-gray-200 rounded-xl focus:border-forest focus:ring-2 focus:ring-forest/10 outline-none transition-all font-medium text-forest" 
                           value="{{ $accountContent->sort_order }}">
                </div>
                ...
                <div class="flex items-center justify-between p-6 bg-forest/5 rounded-2xl border border-forest/10">
                    ...
                    <label class="relative inline-flex items-center cursor-pointer">
                        <input type="checkbox" name="is_active" id="is_active" value="1" class="sr-only peer" {{ $accountContent->is_active ? 'checked' : '' }}>
